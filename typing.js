...
function gameOver() {
    ...
    window.timer = null;
    document.getElementById("info").innerHTML = `Game Over! Your WPM: ${getWPM()}`;
...
document.getElementById("game").addEventListener("keyup", ev => {
    ...
    if (!window.timer && isLetter) {
        window.timer = setInterval(() => {
            ...
            const timeLeft = gameTime - timePassed;
            if (timeLeft <= 0) {
                gameOver();
                return;
            }
            document.getElementById("info").innerHTML = `Time: ${Math.ceil(timeLeft / 1000)}s`;
        }, 1000)
    }
